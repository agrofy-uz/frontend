...
import { getMarketCategories } from '@/shared/api/services/market';
import s from '../../market.module.css';
import {
  getMarketCategoryListHeightPx,
  MARKET_CATEGORY_EXPAND_EXTRA_ROWS,
  MARKET_CATEGORY_GAP_PX,
  MARKET_CATEGORY_ROW_PX,
  MARKET_CATEGORY_ROWS_COLLAPSED,
  MARKET_CATEGORY_TOGGLE_RESERVE_PX,
} from './category.const';
...
function CategoriesBlock({
  ...
  const expandedRowTarget = useMemo(
    () =>
      Math.min(
        MARKET_CATEGORY_ROWS_COLLAPSED + MARKET_CATEGORY_EXPAND_EXTRA_ROWS,
        effectiveCap,
        categories.length
      ),
  ...
  useLayoutEffect(() => {
    ...
    const measure = () => {
      ...
      if (H < 16) return;
      const usable = Math.max(0, H - MARKET_CATEGORY_TOGGLE_RESERVE_PX);
      const unit = MARKET_CATEGORY_ROW_PX + MARKET_CATEGORY_GAP_PX;
      const fit = Math.floor((usable + MARKET_CATEGORY_GAP_PX) / unit);
      const cap = Math.max(
        MARKET_CATEGORY_ROWS_COLLAPSED,
  ...
  useLayoutEffect(() => {
    if (listMoreOpen && expandedRowTarget <= MARKET_CATEGORY_ROWS_COLLAPSED) {
      setListMoreOpen(false);
    }
  ...
  const listMaxHeight = getMarketCategoryListHeightPx(visibleRows);

  const canExpandOnce =
    !listMoreOpen && expandedRowTarget > MARKET_CATEGORY_ROWS_COLLAPSED;
  const canCollapse = listMoreOpen;

  const showListToggle =
    categories.length > MARKET_CATEGORY_ROWS_COLLAPSED &&
    effectiveCap > MARKET_CATEGORY_ROWS_COLLAPSED &&
    expandedRowTarget > MARKET_CATEGORY_ROWS_COLLAPSED;
